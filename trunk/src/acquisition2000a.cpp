...
void Acquisition2000a::get_device_info(device_info_t* info)
{
    ...
    {
        case MODEL_PS2206:
            snprintf(info->device_name, DEVICE_NAME_MAX, "PS2206");
            break;
        case MODEL_PS2207:
            snprintf(info->device_name, DEVICE_NAME_MAX, "PS2207"); 
            break;
        case MODEL_PS2208:
            snprintf(info->device_name, DEVICE_NAME_MAX, "PS2208"); 
            break;
        case MODEL_PS2205MSO:
            snprintf(info->device_name, DEVICE_NAME_MAX, "PS2205MSO"); 
            break;
        case MODEL_NONE:
...
void Acquisition2000a::set_defaults (void)
{
    ...
    {
        ps2000aSetChannel( unitOpened_m.handle,
                           (PS2000A_CHANNEL) PS2000A_CHANNEL_A + ch,
                           unitOpened_m.channelSettings[PS2000A_CHANNEL_A + ch].enabled ,
                           unitOpened_m.channelSettings[PS2000A_CHANNEL_A + ch].DCcoupled ,
                           unitOpened_m.channelSettings[PS2000A_CHANNEL_A + ch].range, 0);
    }
}
...
void Acquisition2000a::collect_block_immediate (void)
{
    ...
    /* Trigger disabled
     */
    ps2000_set_trigger ( unitOpened_m.handle, PS2000A_NONE, 0, PS2000A_RISING, 0, auto_trigger_ms );
...
void Acquisition2000a::collect_block_triggered (trigger_e trigger_slope, double trigger_level)
{
   ...
   */
    unitOpened_m.trigger.simple.channel = PS2000A_CHANNEL_A;
    switch(trigger_slope)
    {
        case E_TRIGGER_FALLING:
            unitOpened_m.trigger.simple.direction = (short) PS2000A_FALLING;
        break;
        case E_TRIGGER_RISING:
        default:
            unitOpened_m.trigger.simple.direction = (short) PS2000A_RISING;
        break;
    }
...
void Acquisition2000a::collect_block_ets (void)
{
    ...
    */
    ...
    unitOpened_m.trigger.simple.direction = PS2000A_RISING;
    ...
    */
    ets_sampletime = ps2000_set_ets ( unitOpened_m.handle, PS2000A_ETS_FAST, 60, 4 );
...
void Acquisition2000a::get_info (void)
  ...
  if( unitOpened_m.handle )
  {
    ...
    switch (variant)
        ...
        unitOpened_m.channelSettings [PS2000A_CHANNEL_A].enabled = 1;
        unitOpened_m.channelSettings [PS2000A_CHANNEL_A].DCcoupled = 1;
        unitOpened_m.channelSettings [PS2000A_CHANNEL_A].range = unitOpened_m.lastRange;

        unitOpened_m.channelSettings [PS2000A_CHANNEL_B].enabled = 0;
        unitOpened_m.channelSettings [PS2000A_CHANNEL_B].DCcoupled = 1;
        unitOpened_m.channelSettings [PS2000A_CHANNEL_B].range = unitOpened_m.lastRange;
  }
  else
  {
...
void Acquisition2000a::set_sig_gen (e_wave_type waveform, long frequency)
{
    ...
    if (frequency < 1000 || frequency > PS2000A_MAX_SIGGEN_FREQ)
    {
...
void Acquisition2000a::set_timebase (double time_per_division)
  ...
  {
      ps2000aGetTimebase( unitOpened_m.handle, i, BUFFER_SIZE, &time_interval, &time_units, oversample, &max_samples, 0 );
#ifdef TEST_WITHOUT_HW
      switch(i)
      {
      case 0:
  ...
  ps2000aGetTimebase( unitOpened_m.handle, timebase, BUFFER_SIZE, &time_interval, &time_units, oversample, &max_samples, 0 );
#endif
